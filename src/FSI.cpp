...
FSI::setup()
{
  // Create the mesh.
  {
    ...
    if (!grid_in_file.is_open())
        {
            std::cerr << "----------------------------------------------------" << std::endl;
            std::cerr << "MESH FILE NOT FOUND" << std::endl;
            std::cerr << "----------------------------------------------------" << std::endl;
            std::abort();
        }
  ...
  // Initialize the finite element space.
  {
    ...
    pcout << "  Pressure degree:           = " << fe_scalar_pressure.degree
          << std::endl;
    pcout << "  Displacement degree:           = " << fe_scalar_displacement.degree
          << std::endl;      
    pcout << "  DoFs per cell              = " << stokes_fe->dofs_per_cell
    ...
    pcout << "  Stokes quadrature points per cell = " << stokes_quadrature->size()
          << std::endl;
    pcout << "  Elasticity quadrature points per cell = " << elasticity_quadrature->size()
          << std::endl;
  ...
  Table<2, DoFTools::Coupling> face_coupling(dim + 1 + dim,
                                             dim + 1 + dim);
  pcout << "Arrived at the mask" << std::endl;
  ...
  std::vector<types::global_dof_index> dofs_per_block_global =
    DoFTools::count_dofs_per_fe_block(dof_handler, block_component);
...
for (unsigned int i = 0; i < 3; ++i)
  for (unsigned int j = 0; j < 3; ++j)
    {
      sparsity.block(i, j).copy_from(dsp.block(i, j));
    }
sparsity.compress();
...
FSI::assemble_system()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    // Debug: Print cell FE info and DoF indices directly
    std::cout << "[DEBUG] Cell idx " << cell->index()
    ...
    for (unsigned int i = 0; i < cell->get_fe().n_dofs_per_cell(); ++i) {
      auto block_idx = cell->get_fe().system_to_block_index(i);
      std::cout << "(" << block_idx.first << "," << block_idx.second << ")";
      if (i + 1 < cell->get_fe().n_dofs_per_cell()) std::cout << ", ";
    }
    std::cout << "]" << std::endl;
    if (!cell_is_in_fluid_domain(cell))
      continue;
...
FSI::solve()
{
  // Simple identity preconditioner that works with block vectors
  // make out of this a serious precoditioner in the hpp file 
  class BlockIdentityPreconditioner
  {
  public:
  ...
  BlockIdentityPreconditioner preconditioner;
...
// check this!
void FSI::assemble_mass_matrix()
{
  // Use hp::FEValues to handle multiple FEs (fluid/solid) as in assemble_system
  hp::QCollection<dim> q_collection_mass;
  q_collection_mass.push_back(*stokes_quadrature);
  q_collection_mass.push_back(*elasticity_quadrature);
  hp::FEValues<dim> hp_fe_values_mass(fe_collection,
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    cell->get_dof_indices(local_dof_indices_mass);
    constraints.distribute_local_to_global(
      cell_mass, local_dof_indices_mass, mass_matrix);
  }
}

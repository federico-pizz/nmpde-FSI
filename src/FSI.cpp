...
FSISerial::setup()
{
  ...
  // Initialize the finite element space.
  {
    ...
    stokes_fe = std::make_unique<FESystem<dim>>(fe_scalar_velocity,
                                         dim,
                                         fe_scalar_pressure,
                                         1,
                                         fe_nothing,
    ...
    elasticity_fe = std::make_unique<FESystem<dim>>(fe_nothing,
                                                    dim,    
                                                    fe_nothing,
                                                    1,
                                                    fe_scalar_displacement,
                                                    dim);

    fe_collection.push_back(*stokes_fe);
    fe_collection.push_back(*elasticity_fe);

    std::cout << "  Velocity degree:           = " << fe_scalar_velocity.degree
          << std::endl;
    std::cout << "  Pressure degree:           = " << fe_scalar_pressure.degree
          << std::endl;
    std::cout << "  Displacement degree:           = " << fe_scalar_displacement.degree
          << std::endl;      
    std::cout << "  DoFs per cell              = " << stokes_fe->dofs_per_cell
    ...
    std::cout << "  Stokes quadrature points per cell = " << stokes_quadrature->size()
          << std::endl;
    std::cout << "  Elasticity quadrature points per cell = " << elasticity_quadrature->size()
          << std::endl;

    common_face_quadrature = std::make_unique<QGauss<dim - 1>>(
        std::max(stokes_fe->degree + 2, elasticity_fe->degree + 2));

    std::cout << "  Quadrature points per face = " << common_face_quadrature->size()
          << std::endl;
  }
  ...
  // Initialize the DoF handler.
  {
    ...
    std::cout << "    total    = " << n_u + n_p << std::endl;
  ...
  // Definition of the BlockMatrix, it's a 3x3 matrix
  
    BlockDynamicSparsityPattern dsp(3, 3);
    dsp.block(0, 0).reinit(n_u, n_u);
    dsp.block(0, 1).reinit(n_u, n_p);
    dsp.block(0, 2).reinit(n_u, n_d);
    dsp.block(1, 0).reinit(n_p, n_u);
    dsp.block(1, 1).reinit(n_p, n_p);
    dsp.block(1, 2).reinit(n_p, n_d);
    dsp.block(2, 0).reinit(n_d, n_u); 
    dsp.block(2, 1).reinit(n_d, n_p);
    dsp.block(2, 2).reinit(n_d, n_d);

    dsp.collect_sizes();
      

  Table<2, DoFTools::Coupling> coupling(dim + 1 + dim, dim + 1 + dim);
  Table<2, DoFTools::Coupling> face_coupling(dim + 1 + dim,
                                             dim + 1 + dim);
  ...
  BlockDynamicSparsityPattern dsp_mass(3, 3);

  dsp_mass.block(0, 0).reinit(n_u, n_u);
  dsp_mass.block(0, 1).reinit(n_u, n_p);
  dsp_mass.block(0, 2).reinit(n_u, n_d);
  
  dsp_mass.block(1, 0).reinit(n_p, n_u);
  dsp_mass.block(1, 1).reinit(n_p, n_p);
  dsp_mass.block(1, 2).reinit(n_p, n_d);

  dsp_mass.block(2, 0).reinit(n_d, n_u);
  dsp_mass.block(2, 1).reinit(n_d, n_p);
  dsp_mass.block(2, 2).reinit(n_d, n_d);

  dsp_mass.collect_sizes();

  DoFTools::make_sparsity_pattern(dof_handler,
                                    coupling,
                                    dsp_mass);
  
  constraints.condense(dsp_mass);

  pressure_mass_sparsity.copy_from(dsp_mass);
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      local_pressure_mass_matrix.reinit(cell->get_fe().dofs_per_cell, cell->get_fe().dofs_per_cell);
      local_matrix = 0;
      local_rhs = 0;
      ...
      if (cell_is_in_fluid_domain(cell))
        {
          constraints.distribute_local_to_global(local_pressure_mass_matrix,
                                             local_dof_indices,
                                             pressure_mass);
        }
